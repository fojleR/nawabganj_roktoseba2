...
class _UpdateProfileState extends State<UpdateProfile> {
      ...
      if (phoneNumberExists) {
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              title: Text("Error"),
              content: Text("User with this phone number does not exist."),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.of(context).pop();
  ...
  Future<void> updateDonationDate(BuildContext context, String phoneNumber) async {
    try {
      // Check if document with the phone number already exists
      bool phoneNumberExists = await FirebaseFirestore.instance
          .collection('Users')
      ...
      if (phoneNumberExists) {
        ...
        await FirebaseFirestore.instance.collection('Users').doc(phoneNumber).update(
          {
            'lastDonationDate': birthDate,
            'lastDonationMonth': birthMonth,
            'lastDonationYear': birthYear,
          }
        );
      } else {
        // Dismiss circular progress indicator dialog
